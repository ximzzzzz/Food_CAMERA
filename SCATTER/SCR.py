...
class Selective_Contextual_refinement_block(torch.nn.Module):
    def __init__(self, input_size, hidden_size, output_size, num_classes, decoder_fix, device):
...
#         self.BiLSTM_2 = torch.nn.LSTM(hidden_size *2, hidden_size, bidirectional = True, batch_first = True)
        ...
        self.selective_decoder = Selective_decoder(input_size*2, num_classes, device )
        self.decoder_fix = decoder_fix

    
    def forward(self, V, label, is_train):
...
#         H, _ = self.BiLSTM_2(H)
        ...
        if (is_train==True) & (self.decoder_fix == False) :
            Probs = self.selective_decoder(D, label)
            return Probs, H
        
        elif (is_train==True) & (self.decoder_fix ==True):
            Probs = self.selective_decoder(D, label)
            return Probs, _
...
#         self.lstm = torch.nn.LSTM(input_size, 1, batch_first= True)
        self.attention_2d = Attention(input_size, num_classes, device=self.device )
    ...
    def forward(self, D, label ):
        ...
        probs = self.attention_2d(D_prime, label)
        return probs
...
#         onehot = torch.FloatTensor(batch_size, onehot_dim).zero_().to(self.device)
    ...
    def forward(self, D_prime, text, is_train=True, batch_max_length = 25):
        batch_size = D_prime.size(0)
        num_steps = batch_max_length +1 # for additional [EOS]
