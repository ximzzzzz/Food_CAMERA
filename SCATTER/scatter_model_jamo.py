...
#         self.AdaptiveAvgPool = nn.AdaptiveAvgPool2d((None,1)) # imgH/16-1   ->  1
        ...
        # CTC DECODER
        self.CTC = CTC.CTC_decoder(opt.output_channel, opt.output_channel, n_top_cls, n_mid_cls, n_bot_cls, device)
...
#         scr_probs_3, H = self.SCR_3(H, text, is_train)
        prob_list = []
        prob_list.append([top_prob, mid_prob, bot_prob])
        
        Prob_list = self.SCR([visual_feature.permute(0, 3, 1, 2).squeeze(3), prob_list,  text, is_train])
