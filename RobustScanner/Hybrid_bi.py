...
#         self.lstm = torch.nn.LSTM(hidden_size, hidden_size, bidirectional=False, num_layers=1, batch_first=True)
...
#         states = (torch.FloatTensor(1, batch_size, self.hidden_size).fill_(0).to(self.device), 
#                   torch.FloatTensor(1, batch_size, self.hidden_size).fill_(0).to(self.device))
...
#                 print('h_t : ', h_t.unsqueeze(2).shape)
                a = torch.softmax(torch.bmm(output, origin_fmap_trans.permute(0, 2, 1)), 2) #shape (N, n_seq, dim), so softmax put effect on dim 2
            ...
            for i in range(self.num_steps):
                ...
                a = torch.softmax(torch.bmm(output, origin_fmap_trans.permute(0,2,1)), 2)
                context = torch.bmm(a, origin_fmap_trans)
